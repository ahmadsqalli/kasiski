...
def nihilist_cipher(decipher, keyword_1, keyword_2, plaintext):
    
    # errors to intercept
        # mapped texts len() doesn't match
        # keyword len() > plaintext len()
        # plaintext is str
    
    plaintext = plaintext.lower()
    
    def keyword_validator(keyword_1):
        #remove duplicate letters
        validated_keyword = []
        for char in list(keyword_1):
            if char not in validated_keyword:
                validated_keyword.append(char)
        return ''.join(validated_keyword)
    def polybius_extraction(text):
        extraction_output = []
        for char in text:
            for row_id in polybius_square:
                if char in polybius_square[row_id]:
                    extraction_output.append(f'{row_id}{polybius_square[row_id].index(char)+1}')
        return extraction_output
        
    validated = keyword_validator(keyword_1)
    new_alphabet = validated + ""
    # generate alphabet for polybius_square
    for char in alphabet:
        if char not in list(validated) and char != 'j':
            new_alphabet += char
    
    #print(new_alphabet)
    # generate polybius_square
    polybius_square = {
        1: list(new_alphabet[0:5]),
        2: list(new_alphabet[5:10]),
        3: list(new_alphabet[10:15]),
        4: list(new_alphabet[15:20]),
        5: list(new_alphabet[20:25])
    }
    
    if decipher == False:
        # convert keyword_2 into numerical format
        numerical_keyword_format = polybius_extraction(keyword_2)
        #print(numerical_keyword_format)
        
        # convert plaintext into numerical format
        plaintext_numerical_format = polybius_extraction(plaintext)
        #print(plaintext_numerical_format)
        
        # map keyword_2 to plaintext
        mapped_keyword_plaintext = numerical_keyword_format*math.ceil((len(plaintext_numerical_format)/len(numerical_keyword_format)))
        #print(mapped_keyword_plaintext[:len(plaintext_numerical_format)])
        
        output = []
        for i, char in enumerate(plaintext_numerical_format):
            output.append(int(char) + int(mapped_keyword_plaintext[i]))
            #print(i, char, mapped_keyword_plaintext[i], int(char) + int(mapped_keyword_plaintext[i]))
